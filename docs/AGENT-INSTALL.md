...
3. Verify with `npx -y biomcp@1.0 doctor` — **exit 0 means you are clear**; exit 1 means read the blockers (each has a `fix_command`).
...
```json
["npx", "-y", "-p", "biomcp@1.0", "-p", "webr@0.6", "-p", "mysql2@3", "biomcp"]
```

**Core + R** (no MySQL):

```json
["npx", "-y", "-p", "biomcp@1.0", "-p", "webr@0.6", "biomcp"]
```

**Minimal (core tools only)** — pinned, still patch-updated:

```json
["npx", "-y", "-p", "biomcp@1.0", "biomcp"]
```
...
```bash
claude mcp add --scope user --transport stdio biomcp -- npx -y -p biomcp@1.0 -p webr@0.6 -p mysql2@3 biomcp
```
...
```json
{
  "mcpServers": {
    "biomcp": {
      "type": "stdio",
      "command": "npx",
      "args": ["-y", "-p", "biomcp@1.0", "-p", "webr@0.6", "-p", "mysql2@3", "biomcp"],
      "env": {}
    }
  }
}
...
```jsonc
{
  ...
  "mcp": {
    "biomcp": {
      "type": "local",
      "command": ["npx", "-y", "-p", "biomcp@1.0", "-p", "webr@0.6", "-p", "mysql2@3", "biomcp"],
      "environment": {},
      "enabled": true,
      "timeout": 120000
    }
...
```bash
codex mcp add biomcp -- npx -y -p biomcp@1.0 -p webr@0.6 -p mysql2@3 biomcp
```
...
{
  "command": "cmd",
  "args": ["/c", "npx", "-y", "-p", "biomcp@1.0", "-p", "webr@0.6", "-p", "mysql2@3", "biomcp"]
}
```
...
```bash
npx -y biomcp@1.0 doctor            # human-readable, exit 1 on blockers
npx -y biomcp@1.0 doctor --json     # machine-readable (schema_version: 1)
```
...
```bash
ANALYSIS_R=1 npx -y -p biomcp@1.0 -p webr@0.6 biomcp doctor
```
...
1. Use a §2 command that carries `-p mysql2@3` — the all-features form already does; the MySQL-only variant is `["npx", "-y", "-p", "biomcp@1.0", "-p", "mysql2@3", "biomcp"]` — or the local-tree + absolute-path alternative shown in §B with `npm install biomcp mysql2`.
...
3. `npx -y biomcp@1.0 doctor` — exit 0 = healthy; read blockers otherwise. **Doctor already proves the post-restart state** — `features[].running_after_restart` and `startup.applied_keys` are computed by replaying the exact startup in a fresh process — so you can *check* without restarting; restart only to *use* the tools in the client.
...
| Upgrade | — | bump the pin in the client config (`biomcp@1.0` → new minor), restart; check `biomcp --version` |
