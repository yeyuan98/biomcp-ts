...
| MySQL | pinned one-shot **as the client command array** (zero-install): `["npx","-y","-p","biomcp@1.1","-p","mysql2@3","biomcp"]` — or a local tree invoked by absolute path: `npm install biomcp mysql2`, then client command `["node","<ABSOLUTE_PATH>/biomcp-mysql/node_modules/biomcp/dist/bundle.js"]` | any directory (the client spawns the server) |
