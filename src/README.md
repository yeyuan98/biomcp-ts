...
│             └───────┬───────┘                                │
│                     ▼                                        │
│              registry.ts (39 sources, incl. patents)         │
│                     │                                        │
│                     ▼                                        │
│            transform/*.ts                                   │
...
`SOURCE_REGISTRY`: `Record<string, ConnectionOptions>` with 39 data source configurations. Organized by domain (genomics, proteins/pathways, drugs, diseases, literature, clinical trials, patents). Each entry specifies URL, protocol (rest/graphql), auth config, and rate limit (including conditional keyed vs. fallback rates).
