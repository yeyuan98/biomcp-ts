...
export function registerBiowasmTools(server: McpServer): void {
  ...
  server.registerTool(
    ...
    {
      description:
        'Reads, depth, or pileup in a genomic region of an alignment (samtools view/depth/mpileup): mode="count" answers "how many reads overlap this locus?" (index-driven, touches only the relevant index blocks); "depth" gives a per-base (optionally binned via depth_bins) coverage profile; "pileup" gives base-level pileup rows; "reads" returns SAM rows or, with format="artifact", a BAM artifact for downstream analysis_bed_op / analysis_biowasm_convert work. Region queries work on any source: indexed sources use fast positional retrieval; indexless sources stream through a synthesized BED region filter (-L/-b/-l). Sortedness: count/reads accept any read order; indexless depth requires coordinate-sorted input and DETECTS order violations (samtools depth silently double-reports them), failing with an actionable error; pileup also assumes sorted input. Use analysis_bam_summary first to see contigs.' +
        SOURCE_NOTES,
      inputSchema: {
        ...SOURCE_INPUT,
        ...REGION_INPUT,
