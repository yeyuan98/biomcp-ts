...
| Tool | Input Schema | Description | Annotations |
|------|-------------|-------------|-------------|
| `article_search` | `query: string`, `source?: "pubmed" \| "europepmc" \| "semantic_scholar" \| "pubtator" \| "litsense" \| "preprint_only" \| "arxiv"`, `limit?: number (1-50, default 10)`, `offset?: number (default 0)`, `dateRange?: string` (YYYY-MM-DD/YYYY-MM-DD, open-ended allowed) | Federated literature search with deduplication and optional date filtering (pubmed, europepmc, semantic_scholar, arxiv in the fan-out; preprint_only explicitly). Note: europepmc truncates date ranges to year-level granularity and caps offset windows at 1000 rows; arXiv clamps page size at 50 and strips quotes/parentheses/field prefixes from the query. `preprint_only` searches ONLY bioRxiv+medRxiv preprints via Europe PMC's PPR index (resultType=core for abstracts + server labels); it is never part of the default federated fan-out. arXiv single-source outages degrade to a friendly `_error` row within the 30 s tool budget (registry timeout 10 s, one retry) | readOnly, openWorld |
| `article_get` | `id: string` (PMID, PMCID, DOI, or bioRxiv/medRxiv preprint DOI — legacy `10.1101/YYYY.MM.DD.NNNNNN` or new `10.64898/*` prefix), `sections?: ("core" \| "oa" \| "annotations" \| "graph" \| "citation" \| "all")[]`, `limit?: number (1-100, default 20)`, `citation_mode?: "fast" \| "full"` (default "fast"), `citation_direction?: "forward" \| "backward" \| "both"` (default "both") | Get detailed article information by identifier. Preprint DOIs return the preprint record from api.biorxiv.org ‖ Europe PMC: all versions, license, category, funders, JATS full-text URL (link only), published-version mapping, and — with `sections=["citation"]` — Europe PMC PPR citations/references (`citation_mode`/`citation_direction` and the `oa`/`annotations`/`graph` sections do not apply to preprints). Journal-article citation: fast mode (~4s, 4 providers — Europe PMC, Semantic Scholar, OpenCitations, Crossref counts/references — with automatic PubMed fallback) or full mode (~15-30s, all 5 providers incl. PubMed). Forward citation lists come from Europe PMC, Semantic Scholar, and OpenCitations; Crossref provides counts and backward references only. Results cached 10min. Journal DOIs are resolved via NCBI IDConv with PubMed esearch fallback | readOnly, openWorld |
