...
- `ensemblConsequence`: VEP compute-on-demand — HGVS c./p./g. via GET `/vep/{species}/hgvs/{url-encoded}`; rsIDs via POST `/vep/{species}/id` (the GET rsID form does not exist upstream). Both forms return an **array** of results (unwrapped to the first entry). Effects sorted by impact severity (HIGH > MODERATE > LOW > MODIFIER), limit-clipped; colocated ClinVar/COSMIC/gnomAD data capped at 5 entries. Note: rsID inputs resolve via dbSNP coordinates and can under-annotate (observed: rs113488060 → upstream_gene_variant, 2 effects, vs missense_variant, 24 effects, for the equivalent HGVS NM_004333:c.1799T>A) — prefer HGVS when both are available
