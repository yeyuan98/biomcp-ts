...
- **Guarded Dependabot auto-merge** (`.github/workflows/dependabot-automerge.yml`) — for `dependabot[bot]` PRs touching only `package-lock.json` with the `dependencies` label: watches the `ci` checks to green (`gh pr checks --watch --fail-fast`) before requesting a merge commit via auto-merge. Safety rests on required-check CI, not diff shape.
