...
- **Packaging** — `webr` is now an optional peer dependency (mysql2 pattern; ~172 MB when opted in) and both esbuild bundle commands mark it `--external:webr` (bundling breaks webR's worker/wasm `__dirname` resolution — verified empirically).
